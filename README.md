...
右上角的图形按钮可在白天、自动、黑夜之间切换，停留时会提示当前模式。自动按 `TZ` 时区的当前时间判断（6:00–18:00 白天），选择会记在浏览器里。照片顺序每次打开会打乱，同一轮滚动里保持稳定。电脑上把鼠标放到一张图上，卡片位置不动，图片只会轻微放大并显示标题、日期和格式等信息。点开大图后，右上角的信息图标可展开完整信息；手机上默认收着。
